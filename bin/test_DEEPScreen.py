...
CHEMBL_CHEMICAL_REPRESENTATIONS_FILENAME = "{}/chembl_32_chemreps.txt".format(test_files_path)
...
def get_act_inact_list_for_a_target_test(target, fl):
    act_list = []
    inact_list = []

    with open("{}/{}/{}".format(test_files_path, target, fl))  as f:
        for line in f:
            print("line ", line)
            if line != "":
                line=line.split("\n")[0]
...
#    print(model)
    ...
    act_list, inact_list = get_act_inact_list_for_a_target_test(target_id, test_filename)
...
#    print("inactives", inact_list)
    ...
    for comp_id in act_list:
        try:
            ...
            pass   
    for comp_id in inact_list:
        try:
            current_df = chem_reps_df[chem_reps_df["chembl_id"] == comp_id]
            compound_smiles = current_df["canonical_smiles"].iloc[0]
            generate_image_from_smiles(comp_id, compound_smiles)
            test_dictionary["test"].append([comp_id, 0])
        except:
            pass        
